...
function updateStatus() {
  if (trial.active) {
    ...
    statusEl.textContent = trial.bonusUsed
      ? "Trial ended — subscribe to continue"
      : "Trial ended — leave a review for +7 days";
...
function renderUsers() {
  ...
  fakeUsers.forEach((u) => {
    ...
    if (trial.active) {
      ...
      buttonHTML = `<button disabled style="opacity:.5">Subscribe</button>`;
    }
...
function bindButtons() {
  ...
  document.querySelectorAll(".review-btn").forEach((btn) => {
    btn.onclick = () => {
      ...
      const ok = confirm(
        "Thanks for supporting Near ❤️\n\nLeave a review and get +7 days free access?"
      );
      ...
      alert("🎉 Thank you! You’ve unlocked 7 extra days.");
    };
  });
}
