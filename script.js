...
function applyVibe(theme){
  ...
  // Keep promo tiles neutral glass (no inherited image)
  setTheme(theme);
}
...
// Deterministic “unique” RandomUser avatars.
// This prevents repeats across vibes and guarantees gender matching.
// Use ids 1..99 (RandomUser has 0..99; we stay inside range)
...
// LOCKED demographic mix (+ tradie + sporty + job levels)
const PEOPLE_BY_VIBE = {
  city: [
    ...
    { name:"David", age:61, gender:"m", bio:"Just chillin • music on", km:4.9, avatarId:73 } // ✅ fixed
  ],
  nature: [
  ...
  fitness: [
    ...
    { name:"Tahlia",age:28, gender:"f", bio:"Pilates • strong + calm", km:3.0, avatarId:33 }, // sporty
  ...
  tech: [
    ...
    { name:"Ben",    age:55, gender:"m", bio:"Tech talk • no ego", km:4.6, avatarId:66 }, // older dude
    ...
    { name:"Casey",  age:37, gender:"m", bio:"IT support • steady vibes", km:0.8, avatarId:25 }, // job level mix
    // Tradie included in tech? better in city. We'll include separate:
  ]
};

// Add a tradie to city (keeps “different job levels”)
PEOPLE_BY_VIBE.city.splice(2, 0, {
  name:"Luke", age:38, gender:"m",
  bio:"Tradie • finishing a job", km:2.2, avatarId:93
});

// Ensure exactly 5 visible for each vibe by slicing later.

function renderPeople(vibe){
  usedAvatarKeys.clear(); // reset so each render is consistent but still unique per list

  const arrRaw = PEOPLE_BY_VIBE[vibe] || PEOPLE_BY_VIBE.city;
