// script.js (ES module) — FULL (6 VIBES + LOCAL IMG + LIVE WAVES)
...
=========================== */
const COPY = {
  tagline: "Real people. Near you.",
  vibeTitle: "Choose your vibe",
  peopleTitle: "People Near",
  liveWavesTitle: "Live Waves",
  waveModalTitle: "Say something real",
  wavePlaceholder: "Write one message..."
};
...
function applyCopy(){
  ...
  if (vibeTitle) vibeTitle.textContent = COPY.vibeTitle;
  if (peopleTitle) peopleTitle.textContent = COPY.peopleTitle;
  if (wavesTitle) wavesTitle.textContent = COPY.liveWavesTitle;

  if (privacyLine) privacyLine.innerHTML = 'Distances shown as a minimum of <b>1 km</b> for privacy.';

  const modalTitle = document.querySelector(".modalTitle");
  if (modalTitle) modalTitle.textContent = COPY.waveModalTitle;

  if (waveText) waveText.placeholder = COPY.wavePlaceholder;
}
